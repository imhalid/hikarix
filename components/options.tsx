    ...
    <div className="flex items-center">
     ...
     <svg className='w-6 h-6 text-white' width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
      ...
      <div
        ...
        className={`flex h-10 w-20 scale-75 items-center rounded-full px-1 transition-all ${infinityValue ? "bg-[#79ca61]" : "bg-neutral-700"}`}
        onClick={handleCheckboxChange}
      >
        <div
          className={`flex h-8 w-8 transform items-center justify-center rounded-full border border-neutral-400 bg-white shadow-md transition-all ${infinityValue ? "translate-x-10" : "translate-x-0"}`}
        >
          {infinityValue ? (
            <svg
              className="h-6 w-6 text-[#436e36]"
              width="512"
              height="512"
              viewBox="0 0 512 512"
     ...
     <svg className='w-6 h-6 text-white' width="512" height="512" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
      ...
      <label className="hidden text-xs text-neutral-100 sm:inline-block">
        Infinity Scroll
      </label>
    </div>
  );
