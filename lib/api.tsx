...
export const GET_MOVIES = async (page: number = 1) => {
  const res = await fetch(
    `https://api.themoviedb.org/3/movie/popular?language=en-US&page=${page}&include_adult=false`,
    TOKEN,
  );
...
export const GET_MOVIE = async (movie_id: string) => {
  const res = await fetch(
    `https://api.themoviedb.org/3/movie/${movie_id}?language=en-US&include_adult=false`,
    TOKEN,
  );
  const data = res.json();
  return data;
...
export const GET_GENRES = async (language: string = "en") => {
  const res = await fetch(
    `https://api.themoviedb.org/3/genre/movie/list?language=${language}&include_adult=false`,
    TOKEN,
  );
  const data = res.json();
  return data;
...
export const GET_UPCOMING = async (page: number = 1) => {
  const res = await fetch(
    `https://api.themoviedb.org/3/movie/upcoming?language=en-US&page=${page}&include_adult=false`,
    TOKEN,
  );
  const { results } = await res.json();
  return results.map((movie: MOVIES_TYPE, index: number) => (
...
export const GET_MOVIE_TRAILER = async (movie_id: string) => {
  const res = await fetch(
    `https://api.themoviedb.org/3/movie/${movie_id}/videos?language=en-US&include_adult=false`,
    TOKEN,
  );
  const data = res.json();
  return data;
...
export const GET_MOVIE_SIMILAR = async (movie_id: string) => {
  const res = await fetch(
    `https://api.themoviedb.org/3/movie/${movie_id}/similar?language=en-US&include_adult=false`,
    TOKEN,
  );
  const { results } = await res.json();
  return results.map((movie: MOVIES_TYPE, index: number) => (
...
export const GET_MOVIE_RELEASE_DATE = async (movie_id: string) => {
  const res = await fetch(
    `https://api.themoviedb.org/3/movie/${movie_id}/release_dates&include_adult=false`,
    TOKEN,
  );
  const data = res.json();
  return data;
...
export const GET_MOVIE_CREDITS = async (movie_id: string) => {
  const res = await fetch(
    `https://api.themoviedb.org/3/movie/${movie_id}/credits?language=en-US&include_adult=false`,
    TOKEN,
  );
  const data = res.json();
  return data;
};
export const GET_MOVIE_BY_GENRE = async (
  page: number = 1,
  genre_id?: number,
) => {
  const res = await fetch(
...
export const GET_COMPANY = async (company_id: string) => {
  const res = await fetch(
    `https://api.themoviedb.org/3/company/${company_id}?language=en-US&include_adult=false`,
    TOKEN,
  );
  const data = res.json();
  return data;
...
export const GET_MOVIE_QUERY = async (page: number = 1, query?: string) => {
  const res = await fetch(
    `https://api.themoviedb.org/3/search/movie?query=${query}&page=${page}&include_adult=false`,
    TOKEN,
  );
  const { results } = await res.json();
  return results.map((movie: MOVIES_TYPE, index: number) => (
...
export const GET_MOVIE_BY_YEAR = async (page: number = 1, year?: number) => {
  const res = await fetch(
    `https://api.themoviedb.org/3/discover/movie?language=en-US&page=${page}&include_adult=false&primary_release_year=${year}`,
    TOKEN,
  );
  const { results } = await res.json();
  return results.map((movie: MOVIES_TYPE, index: number) => (
...
export const GET_DISCOVERY_MOVIE = async ({
  ...
  const res = await fetch(
    `https://api.themoviedb.org/3/discover/movie?with_genres=${genre_id}&page=${page}`,
    TOKEN,
  );
  const data = res.json();
  return data;
