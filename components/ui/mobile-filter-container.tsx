...
export default function Container({ genres, device, children }: { genres: MOVIE_GENRES_TYPE, device: string, children: React.ReactNode }) {
 ...
 return (
   ...
   >
     <div
       className="absolute w-screen h-[200dvh] -translate-x-1/2 -z-30 left-1/2 bg-neutral-950/50 bottom-0 transition-all"
       style={{
         opacity: expanded ? '1' : '0',
         pointerEvents: expanded ? 'auto' : 'none',
       }}
     ></div>
     <motion.div animate={{ paddingBottom: expanded ? '8px' : '30px' }} className="text-white my-4 flex w-full items-center justify-center relative pb-2 ">
       <button onClick={() => setExpanded(!expanded)} className="w-10 h-10 bg-neutral-900 border border-neutral-700 rounded-full flex justify-center items-center">
         <svg className="w-4 h-4" viewBox="0 0 24 24">
           <path fill="currentColor" d="M3 12H21M3 6H21M3 18H21" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
         </svg>
       </button>
     </motion.div>
     <motion.div
     ...
     >
       <p>Genres</p>
       <svg className={`w-4 h-4 transition-all transform ${expanded ? 'rotate-0' : '-rotate-90'}`} viewBox="0 0 24 24">
         <path fill="currentColor" d="M16.59 8.59L12 13.17L7.41 8.59L6 10L12 16L18 10L16.59 8.59Z" />
       </svg>
     </motion.div>
