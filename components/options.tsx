    ...
    <div className="flex items-center">
     ...
     <svg className='w-6 h-6 text-white' width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
      ...
      <div
        ...
        className={`w-20 h-10 scale-75 flex items-center rounded-full transition-all px-1 ${infinityValue ? 'bg-[#79ca61]' : 'bg-neutral-700'}`}
        onClick={handleCheckboxChange}
      >
        <div className={`bg-white w-8 h-8 border border-neutral-400 rounded-full shadow-md flex justify-center items-center transition-all transform ${infinityValue ? 'translate-x-10' : 'translate-x-0'}`}>
          {infinityValue ? (
     ...
     <svg className='w-6 h-6 text-white' width="512" height="512" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
      ...
      <label className="text-neutral-100 hidden text-xs sm:inline-block">Infinity Scroll</label>
    </div>
  )
}