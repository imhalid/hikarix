     ...
     {links.map((link, index) => (
       <div key={index} className="w-fit relative h-full overflow-hidden rounded-2xl inline-flex items-center justify-center">
         ...
         >
           ...
           {path === link.url && (
             <>
               <div className="bg-slate-300 blur-md w-10 h-5 -bottom-2 absolute"></div>
               <div className="bg-slate-300 w-5 h-[2px] rounded-full bottom-0 absolute"></div>
             </>
           )}
           <Link
             href={link.url}
             className={`text-base shadow-xl rounded-2xl text-neutral-200 relative uppercase px-3 py-1 transition-all duration-300 ease-in-out ${path === link.url ? 'font-medium' : 'font-light'}`}
           >
             <div>{link.name}</div>
           </Link>
         </motion.div>
